...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'POST') {
      const { members, groups, orgs, permission, appId } = req.body;
      ...
      if (members && Array.isArray(members)) {
        if (members.length > 0) {
          ...
          if (invalidMembers.length > 0) {
            console.log(
              `[App协作者更新] 忽略无效成员: ${invalidMembers.join(', ')} - 不存在或不属于当前团队`
            );
          }
        }
      }
      ...
      if (orgs && Array.isArray(orgs)) {
        if (orgs.length > 0) {
          ...
          if (invalidOrgs.length > 0) {
            console.log(
              `[App协作者更新] 忽略无效组织: ${invalidOrgs.join(', ')} - 不存在或不属于当前团队`
            );
          }
        }
      }
      ...
      if (groups && Array.isArray(groups)) {
        if (groups.length > 0) {
          ...
          if (invalidGroups.length > 0) {
            console.log(
              `[App协作者更新] 忽略无效成员组: ${invalidGroups.join(', ')} - 不存在或不属于当前团队`
            );
          }
        }
      }
    ...
    console.error('更新App协作者权限失败:', error);
    jsonRes(res, {
      code: 500,
      error: '更新失败'
    });
